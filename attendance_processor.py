...
class AttendanceProcessor:
    ...
    def generate_monthly_summary(self, df_daily: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        summary = df_daily.groupby(['employee_id', 'employee_name']).agg({
            'status': lambda x: {
                'present_days': (x == 'Present').sum(),
                'absent_days': (x == 'Absent').sum(),
                'auto_assigned_days': ((x.str.contains('Auto Assigned|System Assigned', na=False)).sum())
            },
            'worked_hours': 'sum'
        })
    ...
    def process(self, input_file: str, output_file: str, year: int, month: int, 
                max_hours: float = 8.0) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
